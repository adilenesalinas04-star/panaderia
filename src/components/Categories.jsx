...
const categories = [
    ...
    { name: 'Pastries', image: '/assets/prod-donut.png', color: 'bg-[#FFDAC1]' }, // Extra 2
];
    ...
    return (
        <section className="w-full max-w-7xl mx-auto px-8 py-16">
            <div className="flex justify-between items-end mb-10">
                <h2 className="text-3xl md:text-4xl font-bold text-[var(--color-brand-brown)]">Categories</h2>

                <div className="flex gap-2">
                    <button
                        onClick={() => scroll('left')}
                        className="w-10 h-10 rounded-full border border-[var(--color-brand-brown)] text-[var(--color-brand-brown)] flex items-center justify-center hover:bg-[var(--color-brand-brown)] hover:text-white transition-colors cursor-pointer"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <button
                        onClick={() => scroll('right')}
                        className="w-10 h-10 rounded-full bg-[var(--color-brand-brown)] text-white flex items-center justify-center hover:bg-[var(--color-brand-accent)] transition-colors cursor-pointer"
                    >
            ...
            <div
                ref={scrollRef}
                className="flex items-center gap-8 overflow-x-auto pb-8 scrollbar-hide snap-x snap-mandatory"
            ...
            >
                {categories.map((cat, index) => (
                    ...
                    >
                        ...
                        <span className="font-semibold text-[var(--color-brand-brown)] text-lg">{cat.name}</span>
                    </motion.div>
                ))}
            </div>
        </section>
